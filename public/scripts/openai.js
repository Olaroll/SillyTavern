...
function setOpenAIMessages(chat) {
    ...
    // Index of the last user message. Everything after it belongs to the response currently being
    // built, which is the only span where thinking blocks and tool traffic stay valid.
    const lastUserMesIndex = chat.findLastIndex(x => x.is_user);

    for (let i = 0; i < chat.length; i++) {
        const mes = chat[i];
        let role = mes.is_user ? 'user' : 'assistant';
        let content = mes.mes;
        ...
        if (mes.extra?.[IGNORE_SYMBOL]) {
            continue;
        }

        // Thinking blocks, signatures and tool call traffic only stay valid for the duration of a
        // single response: from the user pressing send until the tool chain finishes. Once the turn
        // is complete they're pure bloat, so drop them for anything before the last user message.
        // Doing this here rather than server side keeps the token budget, in-chat injection depths
        // and cache breakpoints all counting the same messages that actually get sent.
        const isActiveTurn = i > lastUserMesIndex;

        // A tool call message carries its payload in extra.tool_invocations; its own content is just
        // a rendered summary that never gets sent. Once the turn is over, there's nothing left to send.
        if (!isActiveTurn && mes.extra?.tool_invocations) {
            continue;
        }
        ...
        switch (oai_settings.names_behavior) {
            ...
            case character_names_behavior.DEFAULT:
                if ((selected_group && mes.name !== name1) || (mes.force_avatar && mes.name !== name1 && mes.extra?.type !== system_message_types.NARRATOR)) {
                    content = `${mes.name}: ${content}`;
                }
                break;
        ...
        const name = mes.name;
        const media = mes.extra?.media;
        const mediaDisplay = getMediaDisplay(mes);
        const mediaIndex = getMediaIndex(mes);

        // Anything still carrying invocations here belongs to the active turn.
        const invocations = mes.extra?.tool_invocations?.slice();

        // Only send thought signatures if they were generated by the same API and model
        const isSameModel = mes.extra?.api === currentApi && mes.extra?.model === currentModel;
        // In group chats, only include reasoning from the currently generating character
        const isOtherGroupMember = selected_group && mes.name !== name2;
        const canSendReasoning = isSameModel && !isOtherGroupMember && isActiveTurn;
        const signature = canSendReasoning ? mes.extra?.reasoning_signature : null;
        const reasoning = canSendReasoning ? String(mes.extra?.reasoning ?? '') : '';
    ...
    // Built oldest first, but the rest of the pipeline expects newest first.
...
async function populateChatHistory(messages, prompts, chatCompletion, type = null, cyclePrompt = null) {
    ...
    for (let index = 0; index < chatPool.length; index++) {
        ...
        if (canUseTools && Array.isArray(chatPrompt.invocations)) {
            ...
            // Only active turn invocations reach this point, so the mode is the only thing left to check.
