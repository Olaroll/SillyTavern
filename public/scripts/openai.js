...
function setOpenAIMessages(chat) {
    ...
    for (let i = chat.length - 1; i >= 0; i--) {
        ...
        const isActiveTurn = j > lastUserMesIndex;
        ...
        if (!isActiveTurn && chat[j].extra?.tool_invocations) {
            j++;
            continue;
        }
        ...
        const signature = canSendReasoning ? chat[j]?.extra?.reasoning_signature : null;
        const reasoning = canSendReasoning ? String(chat[j]?.extra?.reasoning ?? '') : '';
